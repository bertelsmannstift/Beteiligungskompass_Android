...
public class ScrollLayout extends LinearLayout {
    ...
    private void setTime(long time, int loops) {
        ...
        if (loops>2 || currelem.getStartTime() <= time && currelem.getEndTime() >= time) {
            if (loops>2) {
                Util.logDebug(TAG,String.format("time: %d, start: %d, end: %d", time, currelem.getStartTime(), currelem.getEndTime()));
                return;
            }
    ...
    protected void reScrollTo(int x, int y, boolean notify) {
    	if (notify) Util.logDebug(TAG,String.format("scroll to " + x));
        ...
        if (listener!=null && notify) {
            ...
            if (notify) Util.logDebug(TAG,String.format("real time " + currentTime));
            if (notify) Util.logDebug(TAG,String.format(""));
